...
def process_lyrics(artist, track, language, retranslate):
    ...
    download_thread = threading.Thread(target=download_track, args=(artist, track, language)) # Start the MP3 download thread immediately after verifying we have good og lyrics
    ...
    if translated_lyrics:
        ...
        print("Translated lyrics already exist, skipping translation.")
    else:
...
def download_track(artist, track, language):
    ...
    if not os.path.exists(mp3_path):
        try:
            ...
            print(f"MP3 download successful with yt-dlp: {mp3_path}")
...
def main():
    ...
    process_lyrics(artist, track, language, retranslate)     # Process lyrics (this will handle translation and saving).
    print(f"Script completed successfully.")       # Continue further processing with original_lyrics and translated_lyrics as needed.
